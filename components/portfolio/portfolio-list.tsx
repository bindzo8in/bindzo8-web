...
export function PortfolioList({ services }: { services: { id: string, name: string }[] }) {
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="animate-pulse bg-gray-200 aspect-[4/3] rounded-xl" />
          ))}
        </div>
      ) : projects.length > 0 ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project) => (
              <Link href={`/portfolio/${project.slug}`} key={project.id} className="group flex flex-col">
                <div className="relative aspect-[4/3] rounded-xl overflow-hidden mb-4 border border-gray-200">
                  {project.featuredMediaUrl ? (
                    project.featuredMediaUrl.match(/\.(mp4|webm)$/i) ? (
                      <video src={project.featuredMediaUrl} className="w-full h-full object-cover transition-transform group-hover:scale-105" muted autoPlay loop />
                    ) : (
                      <Image src={project.featuredMediaUrl} alt={project.title} fill className="object-cover transition-transform group-hover:scale-105" />
                    )
                  ) : (
                    <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400">No media</div>
                  )}
                  {project.service && (
