  ...
  useGSAP(
    () => {
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        /*
         * Gap between cards (matches `gap-2` on the flex column below).
         * Used so the wrapper travels the *exact* distance needed to
         * bring the next card fully into view, regardless of how many
         * cards there are or how tall each one renders.
         */
        const CARD_GAP_PX = 8;
         ...
         * =========================================================
         *
         * Each card transition shifts the wrapper up by roughly that
         * card's own final height. Summed over every card, that would
         * move the wrapper further than necessary once the container
         * is taller than a single card — there just isn't enough
         * content left below to fill it, so the tail end shows blank
         * space.
         *
         * Instead we cap the TOTAL distance the wrapper travels at
         * exactly (full stack height − visible container height) —
         * the real amount of content that needs to scroll past — and
         * scale every individual shift down proportionally so they
         * still sum to that exact total. Bigger cards still get
         * bigger shifts, just uniformly scaled so the stack never
         * overshoots and leaves the container's bottom empty.
         */

        const cardFinalHeights = cards.map(
          (card, i) => card.offsetHeight + descriptions[i].scrollHeight,
        );

        const totalStackHeight =
          cardFinalHeights.reduce((sum, h) => sum + h, 0) +
        ...
        cards.forEach((card, index) => {
          ...
          if (index < cards.length - 1) {
            /*
             * Move by this card's FINAL height — collapsed height +
             * however much taller it gets once its description expands
             * + the gap — scaled down so the total across every card
             * never exceeds the real scrollable distance (see
             * SHIFT SCALING above).
             */
            const moveAmount =
              (card.offsetHeight + description.scrollHeight + CARD_GAP_PX) *
              shiftScale;
         ...
         * =========================================================
         *
         * Now that every tween (including the final hold above) has
         * been added, tl.duration() reflects the REAL length of the
         * animation. We size the scroll distance off that instead of
         * a flat "cards.length * 1400" guess — that guess didn't
         * scale with the actual amount of animation, so with more
         * cards it over-allocated scroll distance and left the last
         * card's state frozen on screen for extra scroll before
         * unpinning (the "empty space" after the content).
         *
         * PIXELS_PER_DURATION_UNIT keeps the same scroll "feel" as
         * before (tuned to match the original 5-card timing) while
         * now scaling correctly for any number of cards.
         */

        const PIXELS_PER_DURATION_UNIT = 250;

        const scrollDistance = tl.duration() * PIXELS_PER_DURATION_UNIT;
        ...
        const st = ScrollTrigger.create({
          ...
          /*
           * "+=120" delays the pin from grabbing the instant the
           * section's top edge touches the viewport top — you scroll
           * a little further into the section first.
           */
          start: "top top+=120",
